...
####################################################################################################################################################################################
# you need to use numpy and pandas libraries for usage of this code. It is quite trickey to do so with built in python in Blender software. 
# instead of that, please install python directly on your computer and disable path length limit. Than install those packages, for example with cmd commands:
#
# cd LOCATION OF YOUR PYTHON.EXE FILE (usually: C:\Users\USERNAME\AppData\Local\Programs\Python\Python313)
# python.exe -m pip install numpy pandas
#
# After, paste path to libraries into sys.path.append (your path, usually C:/Users/USERNAME/AppData/Roaming/Python/PYTHON_VERSION/site-packages)
####################################################################################################################################################################################

sys.path.append("C:/Users/marti/AppData/Roaming/Python/Python311/site-packages") 
...
# Set up name of collections with analyzed cuts, you can see example structure of .blend file in "Vertical_att_only_extraction_example.blend"
sel = "Example data decimated"

# Set path to the folder where you desire to save your exported data
path_to_save = "O:/000000-My Documents/Shared/du-3DatelierUAM/MK_DATA/09_Zakazky/2024/02Blender_MeshSectionTools/Clanek/GrandFinale" 

####################################################################################################################################################################################
########################################################################Do not chane code below############################################################
####################################################################################################################################################################################
...
# Convert the path_to_save to simplier pth - just part of better interface
pth = (path_to_save + "/" + col.name)
...
columns_to_remove = ['.select_vert', '.select_poly', '.select_edge','Counter_linear', 'Mean Z size H', 'Neck', 'Shoulder', 'Bottom', '.hide_vert' ,'.edge_verts', 'Y_percentage', 'Procesed', '.corner_vert', '.corner_edge', 'Counter', 'position', 'face_normal', 'inside', 'coutner_linear', '_outsideSlice_middle', 'POR', 'Z_percentage', 'Z_percentage_new', 'Radius_int', 'Slice_middle_outside', 'Inside', 'Delete', 'Coding_type', 'Proportional_Z', 'Max_Z_size_of_model', 'Max_Y_section', 'Max_Z_section', 'Partial_Angle', 'X', 'Y', 'Z', 'cutt']  # replace with your column names
...
final_result.to_csv(pth + "\\" + "All_measurment_conected.txt", sep="\t", index=False, decimal=',', float_format='%11.4f')
...
second_row_result.to_csv(pth + "\\" + col.name + "_" + "All_potts.txt", sep="\t", index=False, decimal=',', float_format='%11.4f')
